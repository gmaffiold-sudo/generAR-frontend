...
function CheckoutForm() {
  ...
  // Precargar datos guardados del localStorage
  useEffect(() => {
    try {
  ...
  function handleChange(field: keyof FacturaDatos, value: string) {
    ...
    // Si el usuario edita, resetear la sesión anterior
  ...
  async function handlePagar() {
    ...
    try {
      const res = await apiFetch(`${API}/payments/create-session`, {
        ...
        body: JSON.stringify({
          plan_id:            planId,
          factura_tipo_doc:   datos.tipo_doc,
          factura_numero_doc: datos.numero_doc,
          factura_nombre:     datos.nombre,
          factura_email:      datos.email,
        }),
      });
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: "1 1 320px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {/* ── Sección de pago — siempre visible ── */}
        <div style={{
          ...
          <p style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 14, color: "#7A8EA0", marginBottom: 20, lineHeight: 1.5 }}>
            Serás redirigido a la plataforma de pago segura de Wompi.
          </p>
          ...
          {error && (
            <div style={{
              ...
              <p style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 14, color: "#C04040", lineHeight: 1.5, margin: 0 }}>
                {error}
              </p>
            </div>
          )}

          {/* Botón pagar o widget Wompi */}
          {sessionData ? (
            <>
              <Script
            ...
            <button
              ...
              style={{
                ...
                boxShadow: isComplete ? "0 3px 14px rgba(46,134,171,0.30)" : "none",
            ...
            >
              {loading
                ? <><Spinner size={18} color="#2E86AB" /> Preparando sesión...</>
                : "🔒 Ir a pagar →"
              }
            </button>
          )}
